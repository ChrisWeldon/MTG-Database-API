...
class DataCollector():
    ...
    # @return array of Card objects: card are effectively empty asside from the manifest data
    def get_cards_in_set(self, set_url, rarities=['rare', 'mythic-rare']):
    ...
    def downloadOccData(self, deck_ids):
    ...
    def downloadEventData(self, event_url, agressive=False):
        ...
        while True:
            try:
                data.update({'title': soup.find('h2').text.strip()})
            except Exception as err:
                print(soup)
                continue
            break
...
if __name__ == "__main__":
    ...
    dc = DataCollector()
    db = Database()
    eld = dc.get_cards_in_set("/set/ELD/throne-of-eldraine/")
    db.addCardsToCollection(eld)
